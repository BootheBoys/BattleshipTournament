...
export default function BattleshipModelUpload() {
      ...
      {Array.from({ length: UPLOAD_STEPS }).map((_, index) => (
        <div
          ...
          className={`aspect-square border border-gray-600 rounded ${
            player.hitCells.includes(index) ? "bg-red-500 animate-pulse" : "bg-navy-700"
          }`}
        ></div>
      ))}
    </div>
  ...
  const renderPlayerUpload = (player: "player1" | "player2") => {
    ...
    return (
      <div className="mb-8">
        ...
        <div
          className="border-4 border-gray-600 rounded-lg p-6 bg-navy-800 shadow-lg hover:shadow-yellow-500/50 transition-all duration-300"
        ...
        >
          <div className="text-center mb-4">
            ...
            {playerState.file && <p className="text-yellow-400 mb-4">Selected file: {playerState.file.name}</p>}
          </div>
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-navy-900 p-4">
      <div className="w-full max-w-4xl">
        <h1 className="text-4xl font-bold text-center mb-8 text-yellow-400 animate-pulse">Battleship Model Upload</h1>
